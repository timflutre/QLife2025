...
python ../QLife2025/simulations/src/main.py -savedFolder 'gwas_simu1' -nQTLChr 1 -Lchr 200 -LG 100 -varEffect 1.0 -proportion0 0.0 -varEffect0 0.0 -optim 0 -varW 100 -h2 0.5 -G 1 -N 100 -Npop 10000 -nTrait 1 -nChr 1 -mu 1e-5 >> out_gwas &  
```
...
# genotypes 
geno <- read.table("genotype.txt", header = TRUE, sep = "\t", stringsAsFactors = FALSE, row.names = 1, check.names = FALSE)
...
# snp information
snp <- read.table("SNP_INFO.txt", header = TRUE, sep = "\t")
...
python ../QLife2025/simulations/src/main.py -savedFolder 'gwas_simu2' -nQTLChr 1 -Lchr 20000 -LG 10 -varEffect 1.0 -proportion0 0.0 -varEffect0 0.0 -optim 0 -varW 100 -h2 0.5 -G 1 -N 100 -Npop 10000 -nTrait 1 -nChr 1 -mu 1e-5 >> out_gwas &
```
...
permutation_test <- function(ped, geno, nrep, pval_obs) {
    res <- matrix(0, nrow = nrep, ncol = length(pval))
    for(irep in 1:nrep) {
        pheno <- sample(ped$pheno1)
        lm.res <- apply(geno, 2 , function(x) summary(lm(ped$pheno1 ~ x))$coeff[2,])
        res[irep,] <- pval < min(lm.res[4,])
    }
    res <- apply(res, 2, mean)
    return(res)
}

lm.res$pval2 <- permutation_test(ped, geno, 10, pval_obs = lm.res$pval)
...
python ../QLife2025/simulations/src/main.py -savedFolder 'gwas_simu3' -nQTLChr 1 -Lchr 20000 -LG 100 -varEffect 1.0 -proportion0 0.0 -varEffect0 0.0 -optim 2 -varW 0.01 -h2 0.5 -G 10 -N 100 -Npop 10000 -nTrait 1 -nChr 1 -mu 1e-5 >> out_gwas &
```
...
## only look at the initial generation
indKeep <- which(ped$generation == 10)
ped <- ped[indKeep,]
geno <- geno[indKeep,]
...
plot(100*pca$values / sum(pca$values), xlab = "PC", ylab = "% Variance explained (GRM)")

## add these components in the lm
nPC <- 50 # number of PC to use
# association testing
lm.res <- apply(geno, 2 , function(x) summary(lm(ped$pheno1 ~ pca$vectors[,1:nPC] + x))$coeff[nPC+2,])
# reformat the result
lm.res <- as.data.frame(t(lm.res))
colnames(lm.res) <- c("beta_hat", "se", "tstat", "pval")
lm.res$beta <- snp$beta_trait_1
...
qqplot(-log10(ppoints(nsnp)), -log10(lm.res$pval), xlab = "theoretical", ylab = "observedd", main = "Q-Q Plot for -log10 Pval") 
...
ggplot(lm.res) +
    geom_point(aes(x = snp_id, y = -log10(qval_ash), col = as.factor(qtl))) +
    scale_colour_manual(values = c('0' = "black", '1' = "red"))
...
../../../tests/genpred/LDAK/ldak6.1.linux --make-bed simu2 --gen simu.txt --bim simu.bim --fam simu.fam --gen-skip 0 --gen-headers 0 --gen-probs 1 --threshold 1
...
gemma.res <- read.table("gemma/output/result.param.txt", header = TRUE)
