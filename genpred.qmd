...
We will play here with two simulate data sets. For each, we simulate a population under selection for a polygenic trait, for multiple generations, and we get different types of data, similar to what is usually available from experiments, that we will use in the subsequent analysis:
...
We consider two traits in complete pleiotropy. The effect of their QTLs are correlated (r = 0.5). The first trait has a heritability of 0.5 (trait under selection) while the second has h2 = 0.9. We also consider that these QTLs are linked: 5 chromosomes of 100 cM each, each with 200 QTLs (uniform recombination rate along the chromosomes, the chromosomes are independent). First, a neutral coalescent is used as a burn-in phase (with `msprime`), then 10 generations of selection are done via forward-in-time simulations (`simuPOP`), with 100 individuals being selected.

```{bash}
# conda activate qlife_simu
# python QLife2025/simulations/src/main.py -savedFolder 'GP1' -optim 5 -varW 1 -G 10 -N 100 -Npop 10000 -h2 0.5,0.9 -nTrait 2 -nChr 5 -Lchr 200 -rho 0.005 -mu 1e-4 -proportionQTL 1.0 -varEffect 1.0 -corTrait 0.5
```
...
```{bash}
# conda activate qlife_simu
# python QLife2025/simulations/src/main.py -savedFolder 'GP2' TODO...
```
...
```{r}
inF <- file.path(projectDir, "simulations", "GP1", "pedigree.txt")
...
```{r}
inF <- file.path(projectDir, "simulations", "GP1", "genotype.txt")
stopifnot(file.exists(inF))
...
```{r}
inF <- file.path(projectDir, "simulations", "GP1", "SNP_INFO.txt")
stopifnot(file.exists(inF))
...
```{r}
inF <- file.path(projectDir, "simulations", "GP1", "IBD.txt")
stopifnot(file.exists(inF))
...
#### Download, format and run

```{bash, eval=FALSE}
# Paths
dirLDAK=`pwd`
dirOUT='simulations/GP1' # directory where the output of the simulations are saved

# -------------------------------------------------------------------------------
# Download the LDAK executable

exe=${dirLDAK}"/ldak6.1.linux"
echo ${exe}
if [ ! -f ${exe} ]; then
...
${exe} --make-bed "$dirOUT/"LDAK/simu2 --gen "$dirOUT/"LDAK/simu.txt --bim "$dirOUT/"LDAK/simu.bim --fam "$dirOUT/"LDAK/simu.fam --gen-skip 0 --gen-headers 0 --gen-probs 1 --threshold 1
...
${exe} --cut-weights "$dirOUT/"LDAK/simu_freq --bfile "$dirOUT/"LDAK/simu2
${exe} --calc-weights-all "$dirOUT/"LDAK/simu_freq --bfile "$dirOUT/"LDAK/simu2 

## calculate the GRM
${exe} --calc-kins-direct "$dirOUT/"LDAK/LDAKgmat_freq --bfile "$dirOUT/"LDAK/simu2 --weights "$dirOUT/"LDAK/simu_freq/weights.short --power -.25 --kinship-raw YES
```
...
vcLMM <- function(data, inds, K, Z, colY="pheno1", colZ="ind_id",
  ...
  if(pkg == "rrBLUP"){
    ## prepare the inputs
    if(model == "GBLUP"){
      print("fit a GBLUP model with R/rrBLUP")
...
As explained in [Legarra (2016)](http://dx.doi.org/10.1016/j.tpb.2015.08.005), let $\boldsymbol{u}$ be the vector of genetic values of individuals *in the reference population*, assuming $E[\boldsymbol{u}] = 0$ and $Var(\boldsymbol{u}) = \sigma_u^2 K$.
The genetic variance defined as: $S_u^2 := \frac{1}{n} \sum_i (u_i - \bar{u})^2$. In matrix form: $S_u^2 = \frac{1}{n} \boldsymbol{u}^T \boldsymbol{u} - \frac{1}{n^2} \boldsymbol{1}^T \boldsymbol{u}^T \boldsymbol{u} \boldsymbol{1}$. Because the genetic values are unknown, their genetic variance is unknown, too, and we need to look at its expectation: $E[S_u^2] = \frac{1}{n} \sigma_u^2 tr(K) - \frac{1}{n^2} \sigma_u^2 \boldsymbol{1}^T K \boldsymbol{1} = D_K \; \sigma_u^2$ where $D_K := (\bar{diag(K)} - \bar{K})$.
...
fitG1 <- vcLMM(data=pedDat, inds=as.character(pedDat$ind_id),
               ...
               ## K=GRM_VR1,
               K=as.matrix(Matrix::nearPD(GRM_VR1)$mat),
               method="REML", pkg="rrBLUP")
attr(fitG1, "Vu")
attr(fitG1, "Ve")
...
fitBP1 <- vcLMM(data=pedDat, inds=as.character(pedDat$ind_id),
                Z=Z_VR1, colY="pheno1", colZ="ind_id",
