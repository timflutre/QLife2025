...
They proposed the following vc-LMM with standardized phenotypes: $\boldsymbol{y} = X \boldsymbol{\beta} + \boldsymbol{\epsilon}$ with $\beta_l \sim \mathcal{N}(0, \sigma_l^2)$ and $X_{il} := \frac{M_{il} - 2 f_l}{\sqrt{2 f_l (1 - f_l)}}$ (as in Van Raden's 2nd estimator). That is, each SNP has its own specific variance. Let $h^2_{SNP}$ be the heritability explained by all the SNPs: $\hat{h}^2_{SNP} = \sum_l \hat{\sigma}_l^2$ when LD is ignored. To account for LD, they added SNP-specific weights, so that $\sigma_l^2 = q_l \, h^2_{SNP}$ where $q_l \propto w_l [f_l (1 - f_l)]^{1 + \alpha}$ and $\sum_l q_l = 1$.

Assuming that the unscaled effect-size variance is invariant to the allele frequency ("neutrality") corresponds to $w_l = 1 / L$ and $\alpha = 0$. Assuming that the unscaled effect-size variance increases as the minor allele frequency decreases ("purifying selection") corresponds to $w_l = 1/L$ and $\alpha = -1$. The weights $w_l$ can be computed by the LDAK software, with $\alpha = -0.25$ by default.
...
In practice, $\Sigma_M$ is replaced by its estimate $\hat{\Sigma}_M$, and the theoretical additive genomic variance under the RM-FE model, $\sigma_{RM-FE}^2$, ends up being equal to the (unbiased) *sample* variance of the genomic values, $s_{a,M}^2$. Indeed: $s_{a,M}^2 := \frac{1}{n-1} \sum_i (u_{S,i} - \bar{u}_S)^2$. Given that $\bar{u}_S = 0$ and that $P$ is symmetric and idempotent: $s_{a,M}^2 = \frac{1}{n-1} \boldsymbol{u}_S^T \, \boldsymbol{u}_S = \frac{1}{n-1} \boldsymbol{\beta}^T \, M^T \, P \, M \, \boldsymbol{\beta} = \boldsymbol{\beta}^T \, \hat{\Sigma}_M \, \boldsymbol{\beta}$.
