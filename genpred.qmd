...
Because the genetic values are unknown, their genetic variance is unknown, too. So let us look at its expectation: $E[S_u^2] = \frac{1}{n} \sigma_u^2 tr(K) - \frac{1}{n^2} \sigma_u^2 \boldsymbol{1}^T K \boldsymbol{1} = D_K \; \sigma_u^2$ where $D_K := (\bar{diag(K)} - \bar{K})$.
For the variance component $\sigma_u^2$ of a vc-LMM to directly correspond to the genetic variance in the reference population, $S_u^2$, the relationship matrix $K$ should be made so that $D_K = 1$.
...
\end{align}
where $M$ is the $n \times L$ matrix of SNP genotypes encoded in allele doses (number of copies of an arbitrarily-chosen allele at each SNP) so that $M_{il} \in [0,2]$.

Following [Schreck et al (2019)](https://academic.oup.com/genetics/article/213/2/379/5930631), in matrix form: $GRM_{VR1} := P M M^T P / c_{VR1}$ where $P := \text{Id}_n - \boldsymbol{1}_n \boldsymbol{1}_n^T / n$ is the $n \times n$ matrix used for column-wise mean-centering and $c_{VR1} := 2 \sum_l \hat{f}_l (1 - \hat{f}_l)$.
Not only $P$ happens to be symmetric ($P^T = P$), but it is also idempotent ($P^2 = P$).
...
See the NOIA estimator described in [Vitezica et al (2017)](http://www.genetics.org/content/206/3/1297).
Still, as the authors wrote, "although the NOIA model removes the assumption of HWE, another factor that generates nonindependence between loci and lack of orthogonality is linkage disequilibrium."
...
Following [Schreck et al (2019)](https://academic.oup.com/genetics/article/213/2/379/5930631), similarly, the theoretical **additive genomic variance**, $\sigma_{a,M}^2$, can be (informally) defined as the variance of the genomic value that can be explained by a linear regression on a set of markers, $M$.
One could try to formally define this variance based on the informal definition above. However, as pointed out by Schreck et al, the informal definition does not imply a unique formal definition because of the different modeling possibilities in the vc-LMM: SNP genotypes as random, SNP effects as random, or both as random.

As described above, SNPBLUP models the SNP genotypes as fixed (FM) and the SNP effects as random (RE), so that $Cov(\boldsymbol{u}_S) = \sigma_{\beta}^2 P M M^T P$.
However, this is a variance-covariance matrix whereas the additive genetic variance is a scalar and we would like the additive genomic variance to be so, too.
One way to get a scalar variance from a random vector consists in interpreting the difference between this vector and its expectation as the Euclidean distance, which happens to be equal to the trace of the variance-covariance matrix of the random vector in question.
In the case of the SNPBLUP model (FM-RE), Schreck et al took the average of the trace to be the theoretical additive genomic variance: $\sigma_{a,FM-RE}^2 := \frac{1}{n} tr(Cov(\boldsymbol{u}_S|M)) = \frac{1}{n} \sigma_\beta^2 \, tr(P M M^T P)$. The trace of a matrix being a scalar: \begin{align*}
\sigma_{a,FM-RE}^2 = \frac{n-1}{n} \sigma_\beta^2 \, tr(\hat{\Sigma}_M)
\end{align*}
where $\hat{\Sigma}_M := \frac{1}{n-1} M^T P M$ is the sample variance-covariance matrix of the SNP genotypes.
This shows that, structurally, **the LD between SNPs is ignored in the SNPBLUP model** (FM-RE) as only the trace of $\hat{\Sigma}_M$ appears in the formula. This hence is also the case of the equivalent GBLUP model with Van Raden's estimators: $\sigma_{a,FM-RE}^2 = \frac{1}{n} \sigma_{u,VR1}^2 tr(GRM_{VR1})$.
...
\end{align*}
where $\Sigma_M$ is the variance-covariance matrix of the SNP genotypes.
...
In this RM-FE model, as $\boldsymbol{\beta}$ is random, so is $s_{a,M}^2$.
Schreck et al hence proposed a best predictor for it, as the expectation of $s_{a,M}^2$ conditional on the phenotypic data.
They further showed that it can be partitioned into the unconditional expectation of the additive genomic variance (ignoring LD) and the LD contribution conditional on the phenotypic data, noted $L(\boldsymbol{y})$: $BP(s_{a,M}^2) = \sigma_\beta^2 \, tr(\hat{\Sigma}_M) + L(\boldsymbol{y})$.
In practice, the empirical best predictor is computed using the estimate of $\sigma_\beta^2$ and the eBLUP of $\boldsymbol{\beta}$ obtained by fitting the SNPBLUP model (or its equivalent GBLUP model) with ReML.
Any difference between the unconditional and conditional expectations is due to the LD contribution, whose sign and magnitude depends on the dataset.
