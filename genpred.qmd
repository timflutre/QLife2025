...
\end{align} The additive genetic variance thus not only depends on the contribution of genetic effects and variance of allelic contents (known as *genic variance*), but also on the covariance of allelic contents between QTLs, known as **linkage disequilibrium**. The **narrow-sense heritability** is defined as: $h^2 := \sigma_a^2 / \sigma_p^2$.
...
Estimating variance components requires a **linear mixed model** (LMM) whose generic form is: $\boldsymbol{y} = X \boldsymbol{\beta} + Z \boldsymbol{u} + \boldsymbol{\epsilon}$, where $\boldsymbol{y}$ is the $n$-vector of phenotypes, $X$ is the $n \times p$ design matrix for the explanatory factors whose effects, $\boldsymbol{\beta}$, are modeled as "fixed", $Z$ is the $n \times q$ design matrix for the explanatory factors whose effects, $\boldsymbol{u}$, are modeled as "random", and $\boldsymbol{\epsilon}$ are residual errors. Moreover, $E[\boldsymbol{u}]=0$, $Var(\boldsymbol{u})=G(\gamma)$, $E[\boldsymbol{\epsilon}]=0$, $Var(\boldsymbol{\epsilon})=R(\phi)$, and $Cov(\boldsymbol{u},\boldsymbol{\epsilon})=0$. Caution, the $G$ matrix above does NOT correspond to the genetic variance-covariance matrix between multiple traits as used in evolutionary quantitative genetics.
...
The additive genetic values of unphenotyped individuals can be predicted as long as their additive genetic relationships with phenotyped individuals, $G_{trn,tst}$, are known. Indeed, following [Henderson (1977)](https://linkinghub.elsevier.com/retrieve/pii/S0022030277839350) and [Lopez-Cruz and De Los Campos (2021)](https://academic.oup.com/genetics/article/doi/10.1093/genetics/iyab030/6179494), the data set can be partitioned into a training (trn) and testing (tst) set. The objective is to predict the genetic values of the individuals in the testing set ($\boldsymbol{u}_{tst}$) using the phenotype data available from the training set ($\boldsymbol{y}_{trn}$). After centering, the best linear predictor is: \begin{align}
\hat{\boldsymbol{u}}_{tst} = G^T_{trn,tst} (G_{trn} + \hat{\lambda} I)^{-1} \boldsymbol{y}_{trn}
\end{align} where $\hat{\lambda} := \hat{\sigma}^2 / \hat{\sigma}_u^2$.
...
For each scenario, two correlated traits are simulated. Both traits are in complete pleiotropy, and the effect of their QTLs are correlated. The first trait has a medium heritability whereas the second has a high heritability. In scenarios with selection, only the first trait is directly under selection, the second being indirectly selected.
...
(truth <- list("trait1"=list(varBV_all=var(pedDat[, "bv1"]),
               ...
               "trait1-trait2"=list(corBV_all=cor(pedDat[, "bv1"],
                                                  pedDat[, "bv2"]),
                                    corBV_gen0=cor(pedDat[inds_gen0, "bv1"],
...
As reminded above, the reference population for pedigree relationships is non-ambiguous: it is the base population made of founders, that are assumed unrelated. However, the reference population is not so clear in the case of genomic estimators. When fitting two vc-LMMs, each with a different matrix of genetic relationships, $K_1$ and $K_2$, the estimates of the genetic variance component, $\sigma_{u,1}^2$ and $\sigma_{u,2}^2$, will usually be different. Leaving their uncertainty aside, does the difference only come from the fact that genomic relationships are more accurate? As shown below, notice that all pedigree relationships are positive whereas genomic relationships can be negative. The difference between estimates also come from the fact that both relationships are not on the same scale because they have different reference populations.

![Scheme of various possible reference populations](scheme_pops.png)

Following [Legarra (2016)](http://dx.doi.org/10.1016/j.tpb.2015.08.005), let $\boldsymbol{u}$ be the vector of genetic values of individuals *in the reference population*, assuming $E[\boldsymbol{u}] = 0$ and $Var(\boldsymbol{u}) = \sigma_u^2 K$ where $K$ is the matrix of genetic relationships.

-   The genetic variance is defined as: $S_u^2 := \frac{1}{n} \sum_i (u_i - \bar{u})^2$.

-   In matrix form: $S_u^2 = \frac{1}{n} \boldsymbol{u}^T \boldsymbol{u} - \frac{1}{n^2} \boldsymbol{1}^T \boldsymbol{u}^T \boldsymbol{u} \boldsymbol{1}$.

Because the genetic values are unknown, their genetic variance is unknown, too. So let us look at its expectation: $E[S_u^2] = \frac{1}{n} \sigma_u^2 tr(K) - \frac{1}{n^2} \sigma_u^2 \boldsymbol{1}^T K \boldsymbol{1} = D_K \; \sigma_u^2$ where $D_K := (\bar{diag(K)} - \bar{K})$. For the variance component $\sigma_u^2$ of a vc-LMM to directly correspond to the genetic variance in the reference population, $S_u^2$, the relationship matrix $K$ should be made so that $D_K = 1$.

At Hardy--Weinberg equilibrium, it can be shown that $D_K=1$ for genomic relationship matrices obtained with Van Raden's estimators (see below). These estimators assume that the reference population is a population at HWE with allele frequencies equal to the allele frequencies observed in the sample, and that the locus are at linkage equilibrium. If the sample is indeed at HWE, the diagonal of Van Raden's GRM should be centered on 1 and the off-diagonal on 0.
...
summary(c(GRM_IBD))
summary(diag(GRM_IBD))
summary(GRM_IBD[upper.tri(GRM_IBD)])
...
\hat{GRM}_{ii'}^{(VR1)} = \frac{1}{L} \frac{\sum_{l=1}^L (M_{il} - 2 \hat{f}_l)(M_{i'l} - 2 \hat{f}_l)}{2 \sum_{l=1}^L \hat{f}_l (1 - \hat{f}_l)}
\end{align} where $M$ is the $n \times L$ matrix of SNP genotypes encoded in allele doses (number of copies of an arbitrarily-chosen allele at each SNP) so that $M_{il} \in [0,2]$.

Following [Schreck et al (2019)](https://academic.oup.com/genetics/article/213/2/379/5930631), in matrix form: $GRM_{VR1} := P M M^T P / c_{VR1}$ where $P := \text{Id}_n - \boldsymbol{1}_n \boldsymbol{1}_n^T / n$ is the $n \times n$ matrix used for column-wise mean-centering and $c_{VR1} := 2 \sum_l \hat{f}_l (1 - \hat{f}_l)$. Not only $P$ happens to be symmetric ($P^T = P$), but it is also idempotent ($P^2 = P$).

See the extension to the dominant genetic relationships in [Vitezica et al (2013)](http://dx.doi.org/10.1534/genetics.113.155176).
...
summary(c(GRM_VR1))
summary(diag(GRM_VR1))
summary(GRM_VR1[upper.tri(GRM_VR1)])
...
```{r}
AFs_last <- colMeans(genos[inds_last,]) / 2
plot(AFs_gen0, AFs_last)
plot(AFs_last - AFs_gen0)
```
...
\hat{GRM}_{ii'}^{(VR2)} = \frac{1}{L} \sum_{l=1}^L \frac{(M_{il} - 2 f_l)(M_{i'l} - 2 f_l)}{2 f_l (1 - f_l)}
\end{align}
...
Genomic relationship matrices for genetic values (additive, dominant, epistatic) can be defined in such a way as the genetic variance is partitioned into independent variance components with no covariance between them. As a result, the estimation of one type of genetic (e.g., additive) effects will not be affected by the presence or absence of another type of genetic effects in the model (e.g., epistasis).

See the NOIA estimator described in [Vitezica et al (2017)](http://www.genetics.org/content/206/3/1297). Still, as the authors wrote, "although the NOIA model removes the assumption of HWE, another factor that generates nonindependence between loci and lack of orthogonality is linkage disequilibrium."

### Handling LD with LDAK

As summarized in [Speed et al (2022)](https://onlinelibrary.wiley.com/doi/10.1002/bies.202100170), when SNP genotypes are in linkage disequilibrium, the effect of a causal gene (QTL) ends up being allocated among its tagging SNPs, i.e., non-causal SNPs in LD with the QTL. Speed, Balding and co-workers hence proposed to estimate genomic relatedness with a weight for each SNP, as implemented in the [LDAK software](http://ldak.org). The sum of weights for a set of SNPs decreases as the LD amongst the SNPs increases, so that SNPs in low-LD regions contribute more to SNP heritability than those in high-LD regions.

They proposed the following vc-LMM with standardized phenotypes: $\boldsymbol{y} = X \boldsymbol{\beta} + \boldsymbol{\epsilon}$ with $\beta_l \sim \mathcal{N}(0, \sigma_l^2)$ and $X_{il} := \frac{M_{il} - 2 f_l}{\sqrt{2 f_l (1 - f_l)}}$ (as in Van Raden's 2nd estimator).
That is, each SNP has its own specific variance.
Let $h^2_{SNP}$ be the heritability explained by all the SNPs: $\hat{h}^2_{SNP} = \sum_l \hat{\sigma}_l^2$ when LD is ignored.
To account for LD, they added SNP-specific weights, so that $\sigma_j^2 = q_l \, h^2_{SNP}$ where $q_l \propto w_l [f_l (1 - f_l)]^{1 + \alpha}$ and $\sum_l q_l = 1$.

Assuming that the unscaled effect-size variance is invariant to the allele frequency ("neutrality") corresponds to $w_l = 1 / L$ and $\alpha = 0$.
Assuming that the unscaled effect-size variance increases as the minor allele frequency decreases ("purifying selection") corresponds to $w_l = 1/L$ and $\alpha = -1$.
The weights $w_l$ can be computed by the LDAK software, with $\alpha = -0.25$ by default.
...
summary(c(GRM_ldak))
summary(diag(GRM_ldak))
summary(GRM_ldak[upper.tri(GRM_ldak)])
...
The **ABLUP** model, also called PBLUP, is the following vc-LMM:

```{=tex}
\begin{align}
y = \boldsymbol{\text{1}}_n \, \mu_A + \boldsymbol{u}_A + \boldsymbol{\epsilon}_A
\end{align}
```
-   $\boldsymbol{u}_A \sim \mathcal{N}(\boldsymbol{0}, \sigma_{u,A}^2 \, A)$

    -   $A$: $n \times n$ matrix of additive genetic relationships obtained from the pedigree

    -   $\boldsymbol{u}_A$: $n$-vector of additive genetic values modeled as random

    -   $\sigma_{u,A}^2$: genetic variance component

-   $\boldsymbol{\epsilon}_A \sim \mathcal{N}(\boldsymbol{0}, \sigma^2 \, \text{Id})$.
...
```{=tex}
\begin{align}
y = \boldsymbol{\text{1}}_n \, \mu_G + \boldsymbol{u}_G + \boldsymbol{\epsilon}_G
\end{align}
```
-   $\boldsymbol{u}_G \sim \mathcal{N}(\boldsymbol{0}, \sigma_{u,G}^2 \, GRM)$

    -   $GRM$: $n \times n$ genomic relationship matrix, e.g., from Van Raden's first estimator

    -   $\boldsymbol{u}_G$: $n$-vector of additive genomic values modeled as random
...
The **SNPBLUP** model is the following vc-LMM (it is also known as *ridge regression* in statistics):

```{=tex}
\begin{align}
y = \boldsymbol{\text{1}}_n \, \mu_S + Z \, \boldsymbol{\beta} + \boldsymbol{\epsilon}_S
\end{align}
```
-   $Z$: $n \times L$ matrix of SNP genotypes, *considered fixed* (see below how SNP genotypes should be encoded for SNPBLUP to be equivalent to GBLUP)

-   $\boldsymbol{\beta} \sim \mathcal{N}(\boldsymbol{0}, \sigma_{\beta}^2 \, \text{Id})$

    -   $\boldsymbol{\beta}$: $L$-vector of allelic effects, *modeled as random*

    -   $\sigma_\beta^2$: genetic variance component

-   $\boldsymbol{\epsilon}_S \sim \mathcal{N}(\boldsymbol{0}, \sigma_S^2 \, \text{Id})$.

#### Equivalence of SNPBLUP and GBLUP

Let us define $\boldsymbol{u}_S := Z \boldsymbol{\beta}$ so that $y = \boldsymbol{\text{1}}_n \, \mu_S + \boldsymbol{u}_S + \boldsymbol{\epsilon}_S$.

Following [Schreck et al (2019)](https://academic.oup.com/genetics/article/213/2/379/5930631), for SNPBLUP to be equivalent to GBLUP with Van Raden's first estimator, $\boldsymbol{u}_S$ and $\boldsymbol{u}_G$ should be equal in distribution, which is the case under two conditions:

-   $Z$ should be column-wise mean-centered so that $\bar{\boldsymbol{u}}_S = \bar{\boldsymbol{u}}_G = 0$, which can be achieved by defining $Z := P M$, where $M$ is the $n \times L$ matrix of SNP genotypes encoded in allele doses (number of copies of an arbitrarily-chosen allele at each SNP) so that $M_{il} \in [0,2]$, and $P := \text{Id}_n - \boldsymbol{1}_n \boldsymbol{1}_n^T / n$ is the $n \times n$ matrix used for column-wise mean-centering as defined above for Van Raden's first estimator;

-   in the vc-LMM, SNP genotypes should be modeled as fixed and allelic effects as random, so that $Cov(\boldsymbol{u}_S) := Cov(P M \boldsymbol{\beta}) = P M Cov(\boldsymbol{\beta}) M^T P = \sigma_{\beta}^2 \times c_{VR1} \; GRM_{VR1}$ and thus $\sigma_{u,S}^2 := c_{VR1} \, \sigma_{\beta}^2 = \sigma_{u,G}^2$.

See also [Vitezica et al (2013)](http://dx.doi.org/10.1534/genetics.113.155176).

#### LDAK-based vc-LMM

As the GBLUP above, but with the LDAK genetic relationship matrix as GRM instead.

#### Genetic vs. genomic variance

Theoretically, the **additive genetic variance**, $\sigma_a^2$, is defined as the variance of the additive genetic value based on the QTLs, the QTL effects being considered fixed and the QTL genotypes being considered random. As reminded above, the formal definition takes into account the LD between pairs of QTL genotypes.

Following [Schreck et al (2019)](https://academic.oup.com/genetics/article/213/2/379/5930631), similarly, the theoretical **additive genomic variance**, $\sigma_{a,M}^2$, can be (informally) defined as the variance of the genomic value that can be explained by a linear regression on a set of markers, $M$. One could try to formally define this variance based on the informal definition above. However, as pointed out by Schreck et al, the informal definition does not imply a unique formal definition because of the different modeling possibilities in the vc-LMM: SNP genotypes as random, SNP effects as random, or both as random.

As described above, SNPBLUP models the SNP genotypes as fixed (FM) and the SNP effects as random (RE), so that $Cov(\boldsymbol{u}_S) = \sigma_{\beta}^2 P M M^T P$. However, this is a variance-covariance matrix whereas the additive genetic variance is a scalar and we would like the additive genomic variance to be so, too. One way to get a scalar variance from a random vector consists in interpreting the difference between this vector and its expectation as the Euclidean distance, which happens to be equal to the trace of the variance-covariance matrix of the random vector in question. In the case of the SNPBLUP model (FM-RE), Schreck et al took the average of the trace to be the theoretical additive genomic variance: $\sigma_{a,FM-RE}^2 := \frac{1}{n} tr(Cov(\boldsymbol{u}_S|M)) = \frac{1}{n} \sigma_\beta^2 \, tr(P M M^T P)$. The trace of a matrix being a scalar: \begin{align*}
\sigma_{a,FM-RE}^2 = \frac{n-1}{n} \sigma_\beta^2 \, tr(\hat{\Sigma}_M)
\end{align*} where $\hat{\Sigma}_M := \frac{1}{n-1} M^T P M$ is the sample variance-covariance matrix of the SNP genotypes. This shows that, structurally, **the LD between SNPs is ignored in the SNPBLUP model** (FM-RE) as only the trace of $\hat{\Sigma}_M$ appears in the formula. This hence is also the case of the equivalent GBLUP model with Van Raden's estimators: $\sigma_{a,FM-RE}^2 = \frac{1}{n} \sigma_{u,VR1}^2 tr(GRM_{VR1})$.

When SNP genotypes are modeled as random (RM) and SNP effects as fixed (FE), however, Schreck et al (2019) showed that the theoretical additive genomic variance is the genomic equivalent of the classical additive genetic variance, and that it takes LD into account: $\sigma_{a,M-FE}^2 := var(M \boldsymbol{\beta} | \boldsymbol{\beta})$ so that: \begin{align*} \sigma_{a,RM-FE}^2 = \boldsymbol{\beta}^T \, \Sigma_M \, \boldsymbol{\beta}
\end{align*} where $\Sigma_M$ is the variance-covariance matrix of the SNP genotypes.

In practice, $\Sigma_M$ is replaced by its estimate $\hat{\Sigma}_M$, and the theoretical additive genomic variance under the RM-FE model, $\sigma_{RM-FE}^2$, ends up being equal to the (unbiased) *sample* variance of the genomic values, $s_{a,M}^2$. Indeed: $s_{a,M}^2 := \frac{1}{n-1} \sum_i (u_{S,i} - \bar{u}_S)$. Given that $\bar{u}_S = 0$ and that $P$ is symmetric and idempotent: $s_{a,M}^2 = \frac{1}{n-1} \boldsymbol{u}_S^T \, \boldsymbol{u}_S = \frac{1}{n-1} \boldsymbol{\beta}^T \, M^T \, P \, M \, \boldsymbol{\beta} = \boldsymbol{\beta}^T \, \hat{\Sigma}_M \, \boldsymbol{\beta}$.

In this RM-FE model, as $\boldsymbol{\beta}$ is random, so is $s_{a,M}^2$. Schreck et al hence proposed a best predictor for it, as the expectation of $s_{a,M}^2$ conditional on the phenotypic data. They further showed that it can be partitioned into the unconditional expectation of the additive genomic variance (ignoring LD) and the LD contribution conditional on the phenotypic data, noted $L(\boldsymbol{y})$: $BP(s_{a,M}^2) = \sigma_\beta^2 \, tr(\hat{\Sigma}_M) + L(\boldsymbol{y})$. In practice, the empirical best predictor is computed using the estimate of $\sigma_\beta^2$ and the eBLUP of $\boldsymbol{\beta}$ obtained by fitting the SNPBLUP model (or its equivalent GBLUP model) with ReML. Any difference between the unconditional and conditional expectations is due to the LD contribution, whose sign and magnitude depends on the dataset.

### Implementation

The R function below fits a variance-component linear mixed model (vc-LMM) as written in the reminders above. Two packages can be used, [rrBLUP](https://cran.r-project.org/package=rrBLUP) and [sommer](https://cran.r-project.org/package=sommer), both implementing algorithms for ReML.
...
-   When choosing sommer, the GRM is first computed using Van Raden's first estimator, then the GBLUP model is fitted on phenotypic data scaled to unit variance; finally, the unconditional (named `V`) and conditional expectations (named `W`) of the additive genomic variance from Schreck et al (2019) are returned; more details above.
...
vcLMM <- function(data, inds, K, Z, colY="pheno1", colZ="ind_id", pkg="rrBLUP"){
  ...
  if(pkg == "rrBLUP"){
    ...
    if(model == "GBLUP"){
      ...
      print(paste0("fit a SNPBLUP model with R/rrBLUP (n=", nrow(subData), ")"))
    ...
    print(paste0("fit a GBLUP model with R/sommer",
                 " + Schreck's best predictor",
                 " (n=", nrow(subData), ")"))
    ...
    p <- dim(X)[2]
    c_VR1 <- sum(colMeans(X) * (1 - colMeans(X) / 2))
    GRM <- P %*% tcrossprod(X) %*% P / c_VR1 # eq.3
    ...
    V.hat <- var.g.hat * sum(diag(GRM)) / (n - 1) # eq.16; unconditional estimate
    L.y <- (1/(n-1)) * (sum(g.hat^2) - sum(diag(cov.g.hat))) # p.393 right; LD contrib
    W.hat <- V.hat + L.y # p.393 right; conditional estimate
    attr(subData, "Schreck") <- c("uncond_var"=V.hat,
                                  "uncond.var_plus_var.e"=V.hat + var.e.hat,
                                  "uncond.h2"=V.hat / (V.hat + var.e.hat),
                                  "L.y"=L.y,
                                  "cond_var"=W.hat,
                                  "cond.var_plus_var.e"=W.hat + var.e.hat,
                                  "cond.h2"=W.hat / (W.hat + var.e.hat))
  }
...
### Fit with the true identities by descent
...
```{r, fig.width=10}
fitI1 <- vcLMM(data=pedDat, inds=pedDat$ind_id,
...
```{r, fig.width=10}
fitA1 <- vcLMM(data=pedDat, inds=pedDat$ind_id,
...
```{r, fig.width=10}
fitG1 <- vcLMM(data=pedDat, inds=pedDat$ind_id,
...
```{r, fig.width=10}
fitS1 <- vcLMM(data=pedDat, inds=pedDat$ind_id,
               colY="pheno1", colZ="ind_id",
               Z=Z_VR1, pkg="rrBLUP")
...
```{r}
fitL1 <- vcLMM(data=pedDat, inds=pedDat$ind_id,
               colY="pheno1", colZ="ind_id",
...
Z_trn <- genos_trn - tmp
fit_trn <- vcLMM(data=pedDat, inds=inds_trn,
                 colY="pheno1", colZ="ind_id",
                 Z=Z_trn, pkg="rrBLUP")

dim(genos_tst <- genos[as.character(inds_tst),])
pred_BV1_SNPs <- (genos_tst %*% attr(fit_trn, "BLUP_u"))[,1]
(corP <- cor(pred_BV1_SNPs, setNames(pedDat$bv1, pedDat$ind_id)[names(pred_BV1_SNPs)]))
plot(pred_BV1_SNPs, setNames(pedDat$bv1, pedDat$ind_id)[names(pred_BV1_SNPs)],
     main=paste0("Prediction accuracy with SNP effects",
                 "\ncorP = ", round(corP,3)),
...
```{r}
fitA1 <- vcLMM(data=pedDat, inds=inds_trn,
               K=A, colY="pheno1", colZ="ind_id", pkg="rrBLUP")
...
pred_BV1_A <- (t(G_trn_tst) %*%
                 solve(G_trn + lambda_hat * diag(length(inds_trn))) %*%
                 (y_trn - attr(fitA1, "BLUE_beta")))[,1]
(corP <- cor(pred_BV1_A, setNames(pedDat$bv1, pedDat$ind_id)[names(pred_BV1_A)]))
plot(pred_BV1_A, setNames(pedDat$bv1, pedDat$ind_id)[names(pred_BV1_A)],
     main=paste0("Prediction accuracy with A",
                 "\ncorP = ", round(corP,3)),
     xlab="predicted BV1 (last generation)",
     ylab="true BV1 (last generation)")
```

##### Predict with GRM_IBD

```{r}
fitI1 <- vcLMM(data=pedDat, inds=inds_trn,
               K=GRM_IBD, colY="pheno1", colZ="ind_id", pkg="rrBLUP")

dim(G_trn_tst <- GRM_IBD[inds_trn, inds_tst])
dim(G_trn <- GRM_IBD[inds_trn, inds_trn])
lambda_hat <- attr(fitI1, "Ve") / attr(fitI1, "Vu")
#length(pred_BV1_IBD <- (t(G_trn_tst) %*% solve(G_trn + lambda_hat * diag(length(inds_trn))) %*% y_trn)[,1])
pred_BV1_IBD <- (t(G_trn_tst) %*%
                   solve(G_trn + lambda_hat * diag(length(inds_trn))) %*%
                   (y_trn - attr(fitI1, "BLUE_beta")))[,1]
(corP <- cor(pred_BV1_IBD, setNames(pedDat$bv1, pedDat$ind_id)[names(pred_BV1_IBD)]))
plot(pred_BV1_IBD, setNames(pedDat$bv1, pedDat$ind_id)[names(pred_BV1_IBD)],
     main=paste0("Prediction accuracy with GRM_IBD",
                 "\ncorP = ", round(corP,3)),
     xlab="predicted BV1 (last generation)",
     ylab="true BV1 (last generation)")
```

##### Predict with GRM_VR1

```{r}
fitG1 <- vcLMM(data=pedDat, inds=inds_trn,
               K=GRM_VR1, colY="pheno1", colZ="ind_id", pkg="rrBLUP")
...
pred_BV1_VR1 <- (t(G_trn_tst) %*%
                   solve(G_trn + lambda_hat * diag(length(inds_trn))) %*%
                   (y_trn - attr(fitG1, "BLUE_beta")))[,1]
(corP <- cor(pred_BV1_VR1, setNames(pedDat$bv1, pedDat$ind_id)[names(pred_BV1_VR1)]))
plot(pred_BV1_VR1, setNames(pedDat$bv1, pedDat$ind_id)[names(pred_BV1_VR1)],
     main=paste0("Prediction accuracy with GRM_VR1",
                 "\ncorP = ", round(corP,3)),
     xlab="predicted BV1 (last generation)",
     ylab="true BV1 (last generation)")
```

##### Predict with GRM_ldak

```{r}
fitL1 <- vcLMM(data=pedDat, inds=inds_trn,
               K=GRM_ldak, colY="pheno1", colZ="ind_id", pkg="rrBLUP")

dim(G_trn_tst <- GRM_ldak[inds_trn, inds_tst])
dim(G_trn <- GRM_ldak[inds_trn, inds_trn])
lambda_hat <- attr(fitL1, "Ve") / attr(fitL1, "Vu")
#length(pred_BV1_ldak <- (t(G_trn_tst) %*% solve(G_trn + lambda_hat * diag(length(inds_trn))) %*% y_trn)[,1])
pred_BV1_ldak <- (t(G_trn_tst) %*%
                    solve(G_trn + lambda_hat * diag(length(inds_trn))) %*%
                    (y_trn - attr(fitL1, "BLUE_beta")))[,1]
(corP <- cor(pred_BV1_ldak, setNames(pedDat$bv1, pedDat$ind_id)[names(pred_BV1_ldak)]))
plot(pred_BV1_ldak, setNames(pedDat$bv1, pedDat$ind_id)[names(pred_BV1_ldak)],
     main=paste0("Prediction accuracy with GRM_ldak",
                 "\ncorP = ", round(corP,3)),
     xlab="predicted BV1 (last generation)",
     ylab="true BV1 (last generation)")
```
...
if(FALSE){
  st <- system.time(
    fitA12 <- mmec(fixed = cbind(pheno1, pheno2) ~ 1,
                   random = ~ vsc(id, Gu=A, Gtc=usc(2)),
                   rcov = ~ vsc(units, Gtc=usc(2)),
                   data=subData, nIters=10, verbose=FALSE))
  ## returns an error
  st
  summary(fitA12)
...
```{r mvLMM_VR1, eval=TRUE}
st <- system.time(
  fitG12 <- mmer(fixed = cbind(pheno1, pheno2) ~ 1,
                 random = ~ vsr(id, Gu=GRM_VR1, Gtc=unsm(2)),
                 rcov = ~ vsr(units, Gtc=unsm(2)),
                 data=subData, nIters=10, verbose=FALSE))
st
summary(fitG12)
