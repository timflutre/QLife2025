...
In all scenarios, first, a neutral coalescent is used as a burn-in phase to simulate a population (with `msprime`), then 10 generations of selection are done via forward-in-time simulations (with `simuPOP`).
The population size is constant, with 100 individuals selected at each generation.

For each scenario, two correlated traits are simulated.
Both traits are in complete pleiotropy, and the effect of their QTLs are correlated.
The first trait has a medium heritability and is directly under selection, whereas the second has a high heritability but is indirectly under selection.
...
## Simulation 1: infinitesimal trait, with linkage

```{bash, eval=FALSE}
conda activate qlife_simu
cd simulations

# readme "linkage; polygenic trait"
# python src/main.py -savedFolder 'genpred_simu1' -nQTLChr 1000 -Lchr 1000 -LG 100 -varEffect 1.0 -proportion0 0.0 -varEffect0 0.0 -optim 0 -varW 100 -h2 0.5,0.9 -G 10 -N 100 -Npop 10000 -nTrait 2 -corTrait 0.9 -signCor 'neg' -nChr 5 -mu 1e-5 >> out_genpred &

python src/main.py -savedFolder "genpred_simul1" -optim 0 -varW 100 -G 10 -N 100 -Npop 10000 -h2 0.5,0.9 -nTrait 2 -nChr 5 -Lchr 1000 -nQTLChr 1000 -LG 100 -mu 1e-5 -proportion0 0.0 -varEffect 1.0 -varEffect0 0.0 -corTrait 0.9 -signCor 'neg' >> out_genpred1 &
...
#python src/main.py -savedFolder "genpred_simul1" -optim 0.2 -varW 1 -G 10 -N 100 -Npop 10000 -h2 0.5,0.9 -nTrait 2 -nChr 10 -Lchr 2000 -nQTLChr 100 -LG 100 -mu 1e-5 -proportion0 0.0 -varEffect 1.0 -varEffect0 1.0 -corTrait 0.5 -signCor 'pos' >> out_genpred1 &
```
...
conda activate qlife_simu
cd simulations
python src/main.py -savedFolder "genpred_simul2" TODO...
```
...
image(as(GRM_VR1_not0, "Matrix"), main="GRM (Van Raden 1, all generations except 0)")
...
```{r}
image(as(GRM_ldak, "Matrix"), main="GRM (LDAK, all generations)")
...
image(as(GRM_ldak_gen0, "Matrix"), main="GRM (LDAK, generation 0)")
...
Let us consider that the available phenotypic data are those of generations 0 to 9, and that only the genotyping data are available for generation 10.
A vc-LMM can be trained on the first dataset (generations 0 to 9), and used to predict the breeding values of individuals from generation 10.
